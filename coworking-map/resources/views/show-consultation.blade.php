    ...
    <section id="contact" class="contact">
      ...
      <div class="container" data-aos="fade-up">
        ...
        <div class="uk-panel uk-card uk-card-default uk-card-body">
            ...
            <img class="uk-align-center uk-align-right@m uk-margin-remove-adjacent" src="/image/{{ $consultation->image }}" width="700" alt="Example image">
            <p> <b> Name:</b> {{ $consultation->name }}</p>
            <p> <b> E-mail:</b> {{ $consultation->email }}</p>
            <p> <b> Phone Number:</b> {{ $consultation->phoneNumber }}</p>
            <p> <b> Workspace Name:</b> {{ $consultation->place_name }}</p>
            <p> <b> Address:</b> {{ $consultation->address }}</p>
            <p> <b> Description:</b> {{ $consultation->description }}</p>
            <p> <b> Type:</b> {{ $consultation->type }}</p>
            <p> <b> Options:</b> @foreach($consultation->options as $value){{$value}} @endforeach</p>
            <div>
                <div class="uk-button-group uk-float-right"> 
                    <button class="uk-button uk-button-default">Decline</button>
                    <form method="post" action="{{url('/approve-consultation/'.$consultation->id)}}">
                        {{ csrf_field() }}
